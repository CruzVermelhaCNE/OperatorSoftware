...
<div class="container">
    ...
    <div class="row">
        <div class="col-sm-6">
            ...
            <a href="{{route('goi.edit',$theater_of_operations->id)}}" class="btn btn-dark">Editar
                TO</a>
            <a href="{{route('goi.close',$theater_of_operations->id)}}" class="btn btn-danger">Fechar
                TO</a>
            @else
            <p>Durou
                {{$theater_of_operations->deleted_at->locale('pt_PT')->diffForHumans($theater_of_operations->created_at)}}
            </p>
            <a href="{{route('goi.reopen',$theater_of_operations->id)}}"
    ...
    <div class="row">
        <h4 style="text-align: center;width:100vw;">Fita do Tempo (Resumo) - <a href="{{route('goi.timetape.index', ['type'=>'to','object'=>$theater_of_operations->id])}}">Ver Todo</a></h4>
        ...
        <form style="width:100vw;" method="POST"
            action="{{route('goi.addToTimeTape',['id' => $theater_of_operations->id])}}">
            @csrf
            <div class="row">
    ...
    <div class="row">
        ...
        @if(!$theater_of_operations->trashed())
        <a href="{{route('goi.coordination.create',$theater_of_operations->id)}}"
    ...
    <div class="row">
        ...
        @if(!$theater_of_operations->trashed())
        <a href="{{route('goi.pois.create',$theater_of_operations->id)}}"
    ...
    <div class="row">
        ...
        @if(!$theater_of_operations->trashed())
        <a href="{{route('goi.events.create',$theater_of_operations->id)}}"
    ...
    <div class="row">
        ...
        @if(!$theater_of_operations->trashed())
        <a href="{{route('goi.units.create',$theater_of_operations->id)}}"
            class="btn btn-secondary">Adicionar Meio</a>
        <a href="{{route('goi.units.recreate',$theater_of_operations->id)}}"
    ...
    <div class="row">
        ...
        @if(!$theater_of_operations->trashed())
        <a href="{{route('goi.crews.create',$theater_of_operations->id)}}"
            class="btn btn-secondary">Adicionar Operacional</a>
        <a href="{{route('goi.crews.recreate',$theater_of_operations->id)}}"
    ...
    <div class="row">
        ...
        @if(!$theater_of_operations->trashed())
        <a href="{{route('goi.communication_channels.create',$theater_of_operations->id)}}"
...
<script>
    ...
    function loadObjects(_type) {
        ...
        switch (type) {
            case "to":
                ...
                return;
            case "poi":
                $.get( "{{route('goi.objects.poi',$theater_of_operations->id)}}", function( data ) {
                ...
                break;
            case "event":
                $.get( "{{route('goi.objects.event',$theater_of_operations->id)}}", function( data ) {
                    data.forEach(element => {
                ...
                break;
            case "unit":
                $.get( "{{route('goi.objects.unit',$theater_of_operations->id)}}", function( data ) {
                    data.forEach(element => {
                ...
                break;
            case "crew":
                $.get( "{{route('goi.objects.crew',$theater_of_operations->id)}}", function( data ) {
                    data.forEach(element => {
                        $("#object_selector").append("<option value='"+element.id+"'>"+element.name+"</option>");
                    });
                }, "json" );
    ...
    let list_timetape_table = $('#list_timetape').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.getBriefTimeTape',$theater_of_operations->id)}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "paging":   false,
    ...
    let list_coordination_table = $('#list_coordination').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.getCoordination',$theater_of_operations->id)}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "sort":     false,
    ...
    $('#list_coordination tbody').on( 'click', 'a', function () {
        ...
        let url = "{{route('goi.coordination.edit',['id' => $theater_of_operations->id, 'coordination_id' => '-1'])}}/";
    ...
    let list_pois_table = $('#list_pois').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.getPOIs',$theater_of_operations->id)}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "sort":     false,
    ...
    $('#list_pois tbody').on( 'click', 'a', function () {
        ...
        if(action == "zoom-in") {
            ...
            let url = "{{route('goi.pois.edit',['id' => $theater_of_operations->id, 'poi_id' => '-1'])}}/";
    ...
    let list_events_table = $('#list_events').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.getEvents',$theater_of_operations->id)}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "sort":     false,
    ...
    $('#list_events tbody').on( 'click', 'a', function () {
        ...
        if(action == "zoom-in") {
            ...
            let url = "{{route('goi.events.single',['id'=>$theater_of_operations->id,'event_id' => '-1'])}}";
    ...
    let list_units_table = $('#list_units').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.getUnits',$theater_of_operations->id)}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "sort":     false,
    ...
    $('#list_units tbody').on( 'click', 'a', function () {
        ...
        if(action == "zoom-in") {
            ...
            let url = "{{route('goi.units.single',['id'=>$theater_of_operations->id,'unit_id' => '-1'])}}";
    ...
    let list_crews_table = $('#list_crews').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.getCrews',$theater_of_operations->id)}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "sort":     false,
    ...
    $('#list_crews tbody').on( 'click', 'a', function () {
        ...
        if(action == "zoom-in") {
            ...
            let url = "{{route('goi.crews.single',['id'=>$theater_of_operations->id,'crew_id' => '-1'])}}";
    ...
    let list_communication_channels_table = $('#list_communication_channels').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.getCommunicationChannels',$theater_of_operations->id)}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "sort":     false,
    ...
    $('#list_communication_channels tbody').on( 'click', 'a', function () {
        ...
        let action = $(this).data('action');
        let url = "{{route('goi.communication_channels.edit',['id' => $theater_of_operations->id, 'communication_channel_id' => '-1'])}}";
...
<script>
    ...
    function doneTyping () {
        ...
        axios.post("{{route('goi.updateObservations',$theater_of_operations->id)}}", {
