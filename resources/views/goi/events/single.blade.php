...
<div class="container">
    ...
    <div class="row">
        <div class="col-sm-6">
            ...
            <a href="{{route('goi.single',["id" => $event->theater_of_operations->id])}}"
                class="btn btn-info">Abrir TO</a>
            @if(!$event->isFinished())
            <a href="{{route('goi.events.edit',["id" => $event->theater_of_operations->id, "event_id" => $event->id])}}"
                class="btn btn-dark">Editar</a>
            @endif
            <h2>Status</h2>
            <form
                action="{{route('goi.events.updateStatus',["id" => $event->theater_of_operations->id, "event_id" => $event->id])}}"
                method="POST">
                @csrf
    ...
    <div class="row">
        <h4 style="text-align: center;width:100vw;">Fita do Tempo (Resumo) - <a href="{{route('goi.timetape.index', ['type'=>'event','object'=>$event->id])}}">Ver Todo</a></h4>
    ...
    <div class="row">
        ...
        @if(!$event->isFinished())
        <a href="{{route('goi.events.victims.create',["id" => $event->theater_of_operations->id, "event_id" => $event->id])}}"
    ...
    <div class="row">
        ...
        <form style="width:100vw;" method="POST"
            action="{{route('goi.events.deployUnit',['id' => $event->theater_of_operations->id, 'event_id' => $event->id])}}">
            @csrf
    ...
    <div class="modal-body">
        <div class="row">
            <div class="col-6">
                ...
                <a id="victim_delete" style="display:none" href="{{route('goi.events.victims.delete',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id,'victim_id'=>'-1'])}}" class="btn btn-danger">Apagar Vitima</a>
                @endif
            </div>
            <div class="col-6">
                ...
                @if (!$event->isFinished())
                <form method="POST" action="{{route('goi.events.victims.assignUnit',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id,'victim_id'=>'-1'])}}">
                    @csrf
                    <div class="input-group">
        ...
        <div class="row">
            <div class="col-6">
                ...
                <form method="POST" action="{{route('goi.events.victims.updateStatus',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id,'victim_id'=>'-1'])}}">
                    @csrf
                    <div class="input-group">
...
<script type="text/template" id="template_victim_data">
    ...
    <form metohd="POST" action="{{route('goi.events.victims.updateData',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id,'victim_id'=>'-1'])}}">
        <div class="modal-body">
            @csrf
...
<script type="text/template" id="template_victim_location">
    ...
    <form metohd="POST" action="{{route('goi.events.victims.updateDestination',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id,'victim_id'=>'-1'])}}">
        <div class="modal-body">
                @csrf
                <p>Destino:</p>
...
<script type="text/template" id="template_victim_timings">
    ...
    <form method="POST" action="{{route('goi.events.victims.updateTimings',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id,'victim_id'=>'-1'])}}">
        <div class="modal-body">
            @csrf
            <div class="victim_timings_canceled" style="display:none">
                <p>Cancelado às:</p>
    ...
    <div class="modal-body">
        <div class="row">
            <div class="col-6">
                ...
                <h4>Status</h4>
                <p>Estado Atual: [[STATUSTEXT]]</p>
                <form method="POST" action="{{route('goi.units.updateStatus',['id'=>$event->theater_of_operations->id,'unit_id'=>'-2'])}}">
                    @csrf
                    <div class="input-group">
                ...
                <a href="{{route('goi.units.single',['id'=>$event->theater_of_operations->id,'unit_id' => '-2'])}}" class="btn btn-primary">Abrir Meio</a>
            </div>
            <div class="col-6">
                <h4>Horas</h4>
...
<script type="text/template" id="template_unit_timings">
    ...
    <form metohd="POST" action="{{route('goi.events.units.updateTimings',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id,'event_unit_id'=>'-1'])}}">
        <div class="modal-body">
            @csrf
            <p>Activação:</p>
...
<script>
    ...
    let list_timetape_table = $('#list_timetape').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.events.getBriefTimeTape',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id])}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "paging":   false,
    ...
    let list_victims_table = $('#list_victims').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.events.getVictims',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id])}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "paging":   false,
    ...
    let list_units_table = $('#list_units').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.events.getUnits',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id])}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "sort":     false,
    ...
    function openVictim(id) {
        ...
        closeAllUnitModals();
        let url = "{{route('goi.events.victims.get',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id,'victim_id'=>'-2'])}}";
        ...
        .then(function (response) {
            ...
            function doneTyping () {
                ...
                let url = "{{route('goi.events.victims.updateObservations',['id' => $event->theater_of_operations->id, 'event_id' => $event->id, 'victim_id' => '-1' ])}}".replace("-1",id);
                axios.post(url, {
    ...
    function openUnit(id) {
        closeAllVictimModals();
        closeAllUnitModals();
        let url = "{{route('goi.events.units.get',['id'=>$event->theater_of_operations->id,'event_id'=>$event->id,'event_unit_id'=>'-2'])}}";
        url = url.split("-2").join(id);
        axios.get(url)
        .then(function (response) {
...
<script>
    ...
    function doneTyping () {
        ...
        axios.post("{{route('goi.events.updateObservations',['id' => $event->theater_of_operations->id, 'event_id' => $event->id])}}", {
            observations: observations
        })
        .then(function (response) {
        });
