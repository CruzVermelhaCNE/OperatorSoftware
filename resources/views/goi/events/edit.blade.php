...
<div class="container">
    ...
    <form method="POST"
        action="{{$event?route('goi.events.edit',["id"=>$theater_of_operations->id,"event_id"=>$event->id]):route('goi.events.create',$theater_of_operations->id)}}">
        @else
        @endif
        @csrf
        @if ($event)
        ...
        @if ($theater_of_operations)        
        @if($event)
        <a class="btn btn-info" href="{{route('goi.events.single',["id"=>$theater_of_operations->id,"event_id" => $event->id])}}">Voltar</a>
        @else
