...
<div class="container">
    ...
    <div class="row">
        <div class="col-sm-6">
            ...
            <a href="{{route('goi.crews.demobilize',["id" => $crew->theater_of_operations->id, "crew_id" => $crew->id])}}"
            ...
            @endif
            <a href="{{route('goi.single',["id" => $crew->theater_of_operations->id])}}"
                class="btn btn-info">Abrir TO</a>
            @if ($crew->unit)
            <a href="{{route('goi.units.single',["id" => $crew->theater_of_operations->id, "unit_id" => $crew->unit->id])}}"
                class="btn btn-info">Abrir Meio</a>
            @endif
            @if(!$crew->theater_of_operations->trashed())
            <a href="{{route('goi.crews.edit',["id" => $crew->theater_of_operations->id, "crew_id" => $crew->id])}}"
                class="btn btn-dark">Editar</a>
            @if(!$crew->trashed())
            <h2>Destacar</h2>
            <h5>Meios</h5>
            <form
                action="{{route('goi.crews.assignToUnit',["id" => $crew->theater_of_operations->id, "crew_id" => $crew->id])}}"
                method="POST">
                @csrf
            ...
            <form
                action="{{route('goi.crews.assignToPOI',["id" => $crew->theater_of_operations->id, "crew_id" => $crew->id])}}"
                method="POST">
                @csrf
                <div class="input-group">
    ...
    <div class="row">
        <h4 style="text-align: center;width:100vw;">Fita do Tempo (Resumo) - <a href="{{route('goi.timetape.index', ['type'=>'crew','object'=>$crew->id])}}">Ver Todo</a></h4>
...
<script>
    ...
    let list_timetape_table = $('#list_timetape').DataTable({
        ...
        "ajax": {
            "url": "{{route('goi.crews.getBriefTimeTape',['id'=>$crew->theater_of_operations->id,'crew_id'=>$crew->id])}}",
            "dataSrc": ""
        },
        "order": [[ 0, "desc" ]],
        "paging":   false,
...
<script>
    ...
    function doneTyping () {
        ...
        axios.post("{{route('goi.crews.updateObservations',['id' => $crew->theater_of_operations->id, 'crew_id' => $crew->id])}}", {
