    ...
    <form method="POST"
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="type">
                <option value="A1" @if ($unit) @if($unit->type == "A1") selected @endif @endif>A1</option>
                <option value="A2" @if ($unit) @if($unit->type == "A2") selected @endif @endif>A2</option>
                <option value="B" @if ($unit) @if($unit->type == "B") selected @endif @endif>B</option>
                <option value="C" @if ($unit) @if($unit->type == "C") selected @endif @endif>C</option>
                <option value="VDTD" @if ($unit) @if($unit->type == "VDTD") selected @endif @endif>VDTD</option>
                <option value="TL" @if ($unit) @if($unit->type == "TL") selected @endif @endif>TL</option>
                <option value="CC" @if ($unit) @if($unit->type == "CC") selected @endif @endif>CC</option>
                <option value="CO" @if ($unit) @if($unit->type == "CO") selected @endif @endif>CO</option>
                <option value="LO" @if ($unit) @if($unit->type == "LO") selected @endif @endif>LO</option>
                <option value="LP" @if ($unit) @if($unit->type == "LP") selected @endif @endif>LP</option>
                <option value="HELI" @if ($unit) @if($unit->type == "HELI") selected @endif @endif>HELI</option>
            </select>
        </div>
        <div class="form-group">
            <label>Matricula</label>
            <input type="text" class="form-control" placeholder="Matricula" name="plate" @if ($unit) value="{{$unit->plate}}"
                @endif>
        </div>
        <div class="form-group">
            <label>Nº de Cauda</label>
            <input type="text" class="form-control" placeholder="Nº de Cauda" name="tail_number" @if ($unit)
                value="{{$unit->tail_number}}" @endif>
        </div>
        <div class="form-group">
            <label>Observações</label>
            <textarea class="form-control" placeholder="Observações" name="observations"
                rows="3">@if ($unit){{$unit->observations}}@endif</textarea>
        </div>
        <div class="form-group">
            <label>Estrutura</label>
            <input type="text" class="form-control" placeholder="Estrutura" name="structure" @if ($unit)
                value="{{$unit->structure}}" @endif>
        </div>
        <div class="form-group">
            <label>Latitude da Base</label>
            <input type="text" class="form-control" placeholder="Latitude da Base" name="base_lat" @if ($unit)
                value="{{$unit->base_lat}}" @endif>
        </div>
        <div class="form-group">
            <label>Longitude da Base</label>
            <input type="text" class="form-control" placeholder="Longitude da Base" name="base_long" @if ($unit)
                value="{{$unit->base_long}}" @endif>
        </div>
